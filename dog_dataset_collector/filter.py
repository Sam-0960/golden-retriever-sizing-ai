...
class FlorenceFilter:
    ...
    def load_model(self):
        ...
        try:
            # Note: trust_remote_code=True is required for custom Florence-2 architectures
    ...
    def _run_inference(self, image):
        # We use <MORE_DETAILED_CAPTION> to get a full visual description of the dog crop
...
def run_side_view_filtering(state, state_file):
    ...
    for breed in config.BREEDS.keys():
        ...
        filenames = [f for f in os.listdir(cropped_breed_dir) if f.lower().endswith(('.jpg', '.jpeg', '.png'))]
