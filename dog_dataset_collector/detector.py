...
def detect_and_crop_dogs(state, state_file):
    ...
    device = get_device()
    # Device argument for YOLO should be 'mps', 'cuda', 'cpu', or integer index
    device_str = "cpu"
    if device.type == "mps":
        device_str = "mps"
    ...
    for breed in config.BREEDS.keys():
        ...
        filenames = [f for f in os.listdir(raw_breed_dir) if f.lower().endswith(('.jpg', '.jpeg', '.png'))]
        ...
        for filename in tqdm(filenames, desc=f"Detecting {breed}", unit="img"):
            ...
            try:
                ...
                # We target class 16 (dog) in COCO dataset
