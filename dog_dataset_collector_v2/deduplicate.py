...
def remove_duplicates_and_save(state, state_file):
    ...
    for breed in config.BREEDS.keys():
        ...
        final_breed_dir = os.path.join(config.DATASET_DIR, breed)  # dataset/golden/ etc.
        ...
        filenames = sorted([f for f in os.listdir(filtered_breed_dir) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.webp'))])
        ...
        # Clear out old final folder jpgs
